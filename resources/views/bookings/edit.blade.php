    ...
    <div class="card shadow">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <h3 class="card-title">Edit Booking #{{ $booking->booking_reference }}</h3>
                <div>
                    <a href="{{ route('bookings.show', $booking->id) }}" class="btn btn-outline-info">
            ...
            <form action="{{ route('bookings.update', $booking->id) }}" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="showtime_id" id="showtime_id" class="form-select @error('showtime_id') is-invalid @enderror" required>
                            <option value="">Select Showtime</option>
                            @foreach ($showtimes as $showtime)
                                <option value="{{ $showtime->id }}" {{ old('showtime_id', $booking->showtime_id) == $showtime->id ? 'selected' : '' }}>
                                    {{ $showtime->movie->title }} - {{ $showtime->start_time->format('M d, H:i') }} (Hall: {{ $showtime->hallCinema->name }})
                                </option>
                            @endforeach
                        </select>
                        @error('showtime_id')
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" name="total_amount" id="total_amount" class="form-control @error('total_amount') is-invalid @enderror"
                            value="{{ old('total_amount', $booking->total_amount) }}" step="0.01" min="0" required>
                        @error('total_amount')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" name="booking_fee" id="booking_fee" class="form-control @error('booking_fee') is-invalid @enderror"
                            value="{{ old('booking_fee', $booking->booking_fee) }}" step="0.01" min="0" required>
                        @error('booking_fee')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" name="discount_amount" id="discount_amount" class="form-control @error('discount_amount') is-invalid @enderror"
                            value="{{ old('discount_amount', $booking->discount_amount) }}" step="0.01" min="0">
